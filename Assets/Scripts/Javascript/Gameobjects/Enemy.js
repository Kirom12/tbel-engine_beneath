 ...
 *
 * */
function EnemyObject(ObjectsGrid, gridWidth, gridCaseLength, gridX, gridY, PlayerPropagationObject) 
{
	this.name = "Enemy";
	this.enabled = true;
	this.started = false;
	...
	this.gridCaseLength = gridCaseLength;

	this.ObjectsGrid = ObjectsGrid;

	this.refreshMovement = 500;
	this.Time = Time.Time;

	this.PropagationObject;
	this.PlayerPropagationObject = PlayerPropagationObject;
	...
	{
		if (this.Time + this.refreshMovement < Time.Time)
		{
			this.Time = Time.Time;

			this.ObjectsGrid[this.Transform.Coord.x][this.Transform.Coord.y] = 0;


			if (this.Transform.Coord.x == this.gridWidth - 1)
			{
				this.direction = 'W';
			}
			else if (this.Transform.Coord.x == 0)
			{
				this.direction = 'E';
			}
